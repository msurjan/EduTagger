export interface Yacimiento {
  ...
  nombre_tipo: string;
  sinonimos: string[];
  franja_metalogenica: string;
  region_mvp: string;
  estado: string;
  contexto_regional: string;
  modelo_genetico: string;
  minerales_presentes: { tipo: string; minerales: string[]; descripcion?: string }[];
  rasgos_diagnosticos: { nombre: string; descripcion: string }[];
  material_referencia: { titulo: string; url: string }[];
  profundizacion_opcional: string;
  autor: string;
  validador: string;
  sets: { id_imagen: string; url: string; etiqueta_publica: string; descripcion: string }[];
  etiqueta_publica: string;
  confundibles: { nombre: string; diferencia_clave: string; imagen_placeholder: string }[];
  consigna: string;
  clave_respuesta: {
    alteracion_primaria: string;
    alteracion_sobreimpuesta: string;
    proceso: string;
    periferia: string;
  };
  rubrica: {
    criterios: { item: string; puntaje_max: number; descripcion: string }[];
  };
}

export const mockYacimiento: Yacimiento = {
  id_tipo: "pcu",
  nombre_tipo: "Pórfido Cuprífero",
  sinonimos: ["pórfido de cobre", "pórfido de cobre-molibdeno", "pórfido Cu-Mo"],
  franja_metalogenica: "Franja de pórfidos del Eoceno tardío–Oligoceno temprano del norte de Chile",
  region_mvp: "Norte de Chile (Cordillera)",
  estado: "EN_CURACION",
  contexto_regional: "Los pórfidos están hospedados en una secuencia volcánica-sedimentaria del Paleozoico superior–Triásico inferior (basamento pérmico), con metamorfismo regional de bajo grado (facies esquisto verde inferior) previo a la mineralización. El emplazamiento y la posterior alteración supergénica están controlados estructuralmente (sistema de fallas regional tipo Domeyko / fallas distritales).",
  modelo_genetico: "Sistema de pórfido Cu–Mo formado por intrusiones félsicas-intermedias y fluidos hidrotermales; el cuerpo porfírico de referencia tiene edad ~34 Ma (Oligoceno). Sobre el núcleo porfírico se superponen vetas epitermales de alta sulfuración Cu-Ag-(Au). Posteriormente, lixiviación supergénica genera una zona de enriquecimiento secundario.",
  minerales_presentes: [
    ...
    { tipo: "Zonación", minerales: [], descripcion: "Halo externo rico en pirita / pobre en calcopirita -> núcleo con bornita (+/- digenita) de mayor ley; el Mo se concentra en la parte central/temprana." }
  ],
  rasgos_diagnosticos: [
  ...
  profundizacion_opcional: "El yacimiento Rosario presenta una edad de mineralización hidrotermal y emplazamiento magmático en el límite Eoceno-Oligoceno (aprox. 34.4 Ma). El basamento geológico local influye en la firma geoquímica inicial.",
  autor: "Practicante Graiph / Compilado Literario",
  validador: "CEO Graiph (Pendiente validación de geólogo validador)",
  sets: [
    {
  ...
  etiqueta_publica: "Pórfido Cuprífero del Eoceno-Oligoceno del Norte de Chile",
  confundibles: [
    {
  ...
  rubrica: {
    criterios: [
      { item: "Identificación de Alteraciones", puntaje_max: 3, descripcion: "Identifica correctamente la alteración potásica como primaria y la fílica como sobreimpuesta (1.5 pts c/u)." },
      { item: "Explicación del Proceso Hidrotermal", puntaje_max: 3, descripcion: "Explica el colapso del sistema hidrotermal y el ingreso de fluidos meteóricos tardíos y fríos." },
      { item: "Predicción Periférica", puntaje_max: 4, descripcion: "Predice correctamente la zona propilítica externa y su mineralogía típica (clorita, epidota) con menor ley de cobre." }
    ]
  }
};
